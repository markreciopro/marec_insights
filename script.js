...
/* QUICK INQUIRY — BUILDS A PRE-FILLED MAILTO LINK, NO BACKEND REQUIRED */
(function () {
  'use strict';
  window.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", function (e) {
      ...
      const propertyType = document.getElementById("inquiryPropertyType").value;

      const subject = encodeURIComponent(`MAREC Insights Inquiry — ${interest}`);
      const body = encodeURIComponent(
        "Hi Mark,\n\n" +
        `I'm reaching out about: ${interest}\n` +
        `Property type: ${propertyType}\n\n` +
        "A bit about my property and what's prompting this:\n\n\n" +
        "Thanks,\n"
      );

      window.location.href = `mailto:mark.recio@marec.site?subject=${subject}&body=${body}`;
    });
  });
})();