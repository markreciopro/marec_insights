...
/* VIDEO SPOTLIGHT ENGINE — seamless YouTube loop */
(function () {
  'use strict';
  const mount = document.getElementById("videoSpotlightPlayer");
  if (!mount) return;
  ...
  function createPlayer() {
    player = new YT.Player("videoSpotlightPlayer", {
      videoId: videoId,
      playerVars: {
        autoplay: 1,
        mute: 1,
        loop: 1,
        playlist: videoId,   // required by YouTube so loop:1 works on a single video
        controls: 0,
        playsinline: 1,
        rel: 0,
        modestbranding: 1
      ...
      events: {
        ...
        // Belt-and-suspenders loop: if the native loop param ever
        // fails to restart the video, force it back to the start.
  ...
  if (window.YT && window.YT.Player) {
    ...
    // Load the IFrame API once, then create the player when it's ready.
